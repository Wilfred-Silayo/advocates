            ...
            <div class="card shadow-sm flex-fill">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#viewEventModal" data-id="{{ $event->id }}">
                            View
                        </button>
                        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editEventModal" data-id="{{ $event->id }}">
                            Edit
                        </button>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        @if($event->images)
                        <div id="eventImagesCarouselModal" class="carousel slide">
                            <div class="carousel-inner">
                                @foreach($event->images as $index => $image)
                                <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                    <img src="{{ asset('storage/'.$image) }}" class="d-block w-100" alt="Event Image">
                            ...
                            <button class="carousel-control-prev" type="button" data-bs-target="#eventImagesCarouselModal" data-bs-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                <span class="visually-hidden">Previous</span>
                            </button>
                            <button class="carousel-control-next" type="button" data-bs-target="#eventImagesCarouselModal" data-bs-slide="next">
                                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                <span class="visually-hidden">Next</span>
                            </button>
                        </div>
...
<script>
    $(document).ready(function() {
        let deleteForm = $('#deleteEventForm');
        $('.delete-btn').click(function() {
            let eventId = $(this).data('id');
            deleteForm.attr('action', '/events/' + eventId);
        ...
        $('#viewEventModal').on('show.bs.modal', function(event) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#viewEventModal .modal-body').html(`
                        <h5 class="card-title">${data.name}</h5>
                        <p class="card-text">${data.content}</p>
                        <p class="text-muted">Event Date: ${data.event_date}</p>
                        ${data.images ? `
                        <div id="eventImagesCarouselModal" class="carousel slide">
                            <div class="carousel-inner">
                                ${data.images.map((image, index) => `
                                <div class="carousel-item ${index === 0 ? 'active' : ''}">
                                    <img src="/storage/${image}" class="d-block w-100" alt="Event Image">
                                </div>
                                `).join('')}
                            </div>
                            <button class="carousel-control-prev" type="button" data-bs-target="#eventImagesCarouselModal" data-bs-slide="prev">
